          ...
          {histories.map((listing, key) => {
            return (
              <div key={key} className='property-histories-item'>
                <div className='first'>
                  <div className='first-wrapper'>
                    ...
                    {(listing.status === 'U' && listing.lastStatus === 'Sld') ? null : (
                      <span style={{ fontSize: 12, fontWeight: '500', color: '#4E9EE9', textDecorationLine: 'underline', marginLeft: 10, cursor: 'pointer' }} onClick={() => onDetail(listing.id, listing.streetNumber, listing.streetName, listing.streetSuffix, listing.mlsNumber)}>View</span>
                    )}
                  </div>
                ...
                <div className='property-histories-price'>
                  <span style={{ fontSize: 12, fontWeight: '500', color: '#4E9EE9', textDecorationLine: 'none', marginLeft: 10, cursor: 'pointer' }} onClick={() => onDetail(listing.id, listing.streetNumber, listing.streetName, listing.streetSuffix, listing.mlsNumber)}>{listing.mlsNumber}</span>
                </div>
              </div>
            );
          })}
