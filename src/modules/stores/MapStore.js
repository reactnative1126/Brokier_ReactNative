...
class Store {
  ...
  onSearchFilter(property) {
    let searchString = window.searchString;
    let flag = true;
  ...
  getMarkers(listings) {
    ...
    for (let index = 0; index < listings.length; index++) {
      ...
      const key = window.zoom < 15 ? listingOne.latitude + '#' + listingOne.longitude : listingOne.streetNumber + listingOne.streetName + listingOne.streetSuffix;
